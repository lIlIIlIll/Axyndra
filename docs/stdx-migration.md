...
`model_adapters` 负责把 Agent request/response 投影到 llm4cj wire DTO，解析凭据，默认校验 HTTPS/loopback URL；Provider Profile 可显式 opt-in 远程 HTTP，绑定 request ID，维护总 deadline 和取消，控制 64 MiB response 上限，脱敏错误和流片段，并决定 retry/fallback。Wirestack 只提供通用 HTTP/TLS/response body/cancellation primitive，不认识 `ProviderSpec`、Agent error code 或 provider 名称。
...
| TLS/trust | 默认 `HttpClient` 惰性使用 `systemDefaultAlpn()` / `systemDefault()`；显式 custom roots 或 mTLS 使用 `HttpClientTlsConfig` | 不复制系统 CA 发现逻辑，也不新增下层 system-trust 入口。`model_adapters` 继续拥有 HTTPS/loopback URL policy、显式远程 HTTP opt-in、凭据和错误脱敏；只有显式 custom roots/mTLS 时才配置下层 TLS。 | 上层可适配（仅源码证据）；真实信任链和 hostname 校验未验证。 | 当前：`model_adapters/src/native_transport.cj`；候选：`../Wirestack/src/http/client.cj`、`../Wirestack/src/trust.cj` 的 `TrustPolicy.system`、`../Wirestack/src/http/tls.cj` 的 `HttpClientTlsConfig`。 |
...
| --- | --- | --- | --- | --- |
| JsonKind、string/bool/numberText | yjson JsonKind.Null/Boolean/Number/String/Array/Object；JsonValueView.asString/asBool/asNumberText；JsonNode 静态构造器 | yjson_support 继续提供 UnifiedJsonKind、text、boolValue、numberText，业务代码继续使用 AgentValue = JsonNode。 | 已通过 yjson 0.1.0 tag 的 clean check、workspace unit tests 和产品合同验证。 | 当前：libs/yjson_support/src/json_support.cj |
| YJson.parse + JsonReadConfig | JsonNode.parse(text, config: JsonReadOptions)，也有 bytes 入口 | parseUnifiedJson 把旧 maxBytes 映射到 maxInputBytes、maxStringBytes、maxBufferedValueBytes，保留 Reject duplicate、Preserve number literal 和 maxDepth；不保留旧配置 alias。 | 已通过 yjson_support、jsonrpc4cj 和产品回归合同验证。 | 当前：libs/yjson_support/src/json_support.cj；release：yjson 0.1.0 |
| YJson.stringify/value/nullValue 与旧构造器 | JsonNode.toJson()；JsonNode.boolean/string/number/array/object；JsonNode.nullValue | 数字使用文本入口，禁止经 Float64 往返；对象字段顺序由上层保留；jsonObject 在 Axyndra seam 拒绝 duplicate key。 | 已通过 workspace unit tests、JSON-RPC 数字 ID 和产品回归合同验证。 | 当前：libs/yjson_support/src/json_support.cj；release：yjson 0.1.0 |
| 对象/数组内部容器 | 公开 JsonArray.size/get/add；JsonObject.size/nameAt/valueAt/put | 不调用 yjson 包内 values()/entries()，也不要求 yjson 暴露 backing storage。UnifiedJsonAccess.items 继续保持 live alias。 | 已通过 yjson_support 和依赖该 seam 的 workspace tests 验证。 | 当前：libs/yjson_support/src/json_support.cj；release：yjson 0.1.0 |
| schema | yjson_support 内嵌的 JsonSchema.fromValue(JsonValueView)；validate(JsonValueView): JsonSchemaResult；JsonSchemaViolation.instancePath/schemaPath/code/message | yjson_support.validateUnifiedJsonSchema 把 violation 映射回既有 path/keyword/message。保留 required 的 $ 与类型错误的 $["name"] 语义，不要求通用库直接输出产品 JSONPath。 | 已通过 yjson 0.1.0 tag 算法源码和 yjson_support 合同测试验证。 | 当前：libs/yjson_support/src/json_support.cj 及其内嵌算法源码 |
